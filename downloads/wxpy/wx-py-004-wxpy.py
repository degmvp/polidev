        ...
        for field, rules in self._schema.items():
            ...
            if isinstance(value, str):
                if "min_length" in rules and len(value) < rules["min_length"]:
                    errors.append(ValidationError(field_path, f"Comprimento mínimo: {rules['min_length']}"))
                if "max_length" in rules and len(value) > rules["max_length"]:
                    errors.append(ValidationError(field_path, f"Comprimento máximo: {rules['max_length']}"))
                if "pattern" in rules and not re.match(rules["pattern"], value):
                    errors.append(ValidationError(field_path, "Formato inválido"))

            if isinstance(value, (int, float)):
                if "min_value" in rules and value < rules["min_value"]:
                    errors.append(ValidationError(field_path, f"Valor mínimo: {rules['min_value']}"))
                if "max_value" in rules and value > rules["max_value"]:
                    errors.append(ValidationError(field_path, f"Valor máximo: {rules['max_value']}"))

            if "choices" in rules and value not in rules["choices"]:
                errors.append(ValidationError(field_path, f"Valor deve ser um de: {rules['choices']}"))

            if "validator" in rules and not rules["validator"](value):
                errors.append(ValidationError(field_path, "Validação customizada falhou"))
            ...
            if "items" in rules and isinstance(value, list):
                for i, item in enumerate(value):
                    ...
                    if "type" in item_rules and not isinstance(item, item_rules["type"]):
                        errors.append(ValidationError(item_path, "Tipo de item inválido"))
...
# })
#
# dados = {"nome": "Maria Silva", "email": "maria@exemplo.com", "idade": 28, "role": "admin"}
