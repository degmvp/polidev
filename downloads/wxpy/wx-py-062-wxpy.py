...
class RateLimiter:
    ...
    Args:
        rate: Número de operações permitidas por segundo.
        burst: Capacidade máxima do bucket (rajadas permitidas).
    """
    ...
    def acquire(self, timeout: float | None = None) -> bool:
        """
        Adquire permissão para executar. Bloqueia até obter ou timeout.

        Args:
            timeout: Tempo máximo de espera em segundos. None = espera infinita.

        Returns:
            True se adquiriu, False se timeout expirou.
        """
    ...
    def try_acquire(self) -> bool:
        """Tenta adquirir sem bloquear. Retorna False se não há tokens."""
        with self._lock:
            self._refill()
            if self._tokens >= 1.0:
                self._tokens -= 1.0
...
def rate_limited(rate: float, burst: int | None = None):
    """
    Decorator que aplica rate limiting a uma função.

    Args:
        rate: Operações por segundo permitidas.
    ...
    def decorator(func: Callable) -> Callable:
        ...
        wrapper.limiter = limiter  # type: ignore
        return wrapper
...
# ===================== EXEMPLO DE USO =====================
#
# # Como decorator: máximo 5 chamadas por segundo
...
#
# print(limiter.stats)
# >>> {'total_requests': 100, 'total_throttled': 0, ...}
