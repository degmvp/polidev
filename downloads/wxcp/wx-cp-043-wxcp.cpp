// wx-cp-043-wxcp.cpp
// Migrado do acervo legado: cppb04.cpp
// POLYDEV | WXCP
// Título: Rotação bitwise para criptografia simples
...
std::uint32_t encrypt(std::uint32_t v){return rotateLeft(v,5)^0xA5A5A5A5u;} std::uint32_t decrypt(std::uint32_t v){return rotateRight(v^0xA5A5A5A5u,5);} int main(){std::uint32_t x=123456,e=encrypt(x); std::cout<<"Original: "<<x<<" Encriptado: "<<e<<" Decriptado: "<<decrypt(e)<<'\n';}
